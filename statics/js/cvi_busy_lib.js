...
function showLoading(msg) {
	if (loadingHUD == null) {
		loadingHUD = getBusyOverlay('viewport', { color: 'black', opacity: 0.3, text: 'viewport: loading...', style: 'text-shadow: 0 0 3px black;font-weight:bold;font-size:16px;color:white' }, { color: '#000', size: 100, type: 'o' });
	}
	if (loadingHUD) {
		loadingHUD.settext(msg);
	}
}
...
function getBusyOverlay(parent, overlay, busy) {
	if ((typeof (parent) === 'object' || parent == 'viewport') && document.getElementsByTagName) {
		...
		if (parent == 'viewport') {
			...
			if (!viewport) {
				viewport = document.createElement('div'); viewport.id = 'viewport'; cz = viewport.style;
